...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isIntroSeen || document.documentElement.classList.contains('intro-skipped')) {
    skipIntroInstantly();
  ...
  function initIntroAnimation() {
    ...
    function spawnHackingAlert() {
      ...
      if (rand < 0.70) {
        ...
        details.textContent = errorCodes[Math.floor(Math.random() * errorCodes.length)];
    ...
    tl.to(progressObj, {
      ...
      onUpdate: () => {
        ...
        // 進行度に応じたログ変更
        if (val > 45 && val < 85 && statusText.textContent !== "CONNECTING...") {
          statusText.textContent = "CONNECTING...";
    ...
    .to({}, {
      ...
      onStart: () => {
        ...
        statusText.style.textShadow = "0 0 10px rgba(255, 255, 255, 0.8), 0 0 20px #00f3ff";
      }
    })
  ...
  if (customCursor) {
    const interactives = selectAll('a, button, input, select, textarea, .gallery-card, .sponsor-logo-box, .cf-turnstile');
  ...
  const validateField = (input) => {
    ...
    else if ((input.id === 'portfolio' || input.type === 'url') && input.value.trim()) {
      try {
        const parsedUrl = new URL(input.value.trim());
        if (parsedUrl.protocol !== 'https:') {
          isValid = false;
          errorMsg = 'セキュリティ保護のため、https:// で始まるURLのみ入力可能です。';
        }
      } catch (_) {
        isValid = false;
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', async (e) => {
      ...
      if (!turnstileResponse || !turnstileResponse.value) {
        isFormValid = false;
        turnstileErrorSpan.textContent = 'セキュリティ認証（Botチェック）を完了してください。';
      } else {
      ...
      if (!isFormValid) {
        ...
        formAlert.textContent = '入力内容、またはセキュリティ認証に不備があります。';
      ...
      try {
        ...
        if (response.ok && result.success) {
          ...
          formAlert.textContent = result.message || 'エントリーを受け付けました。ご応募ありがとうございます。';
          formAlert.style.display = 'block';
        ...
        formAlert.className = 'form-alert error';
        formAlert.textContent = error.message || '接続エラーが発生しました。再度お試しください。';
        formAlert.style.display = 'block';
